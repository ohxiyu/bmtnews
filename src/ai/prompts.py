...
结构（2-4 段，段与段之间空一行）：
1. 第一段直接说最重要的事实或判断，不用新闻腔，不做情绪铺垫。
2. 中段按时间或因果顺序补足关键动作、数字和机制，每句话都提供新信息。
3. 最后停在当前状态；只有材料明确支持时，才加一句简短影响，不强制升华。

硬性要求：
- 全文 180-300 个汉字；优先删重复背景、次要数字、形容词和推演，不删主体、事件与当前状态
...
X_POST_USER = """把下面这条新闻写成一条 180-300 字的中文紧凑信息推。
