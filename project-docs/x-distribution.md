...
`compose: "ai"`（默认）时，每条推由 AI 按账号口吻单独成文。写的是**叙事型长推**：
读者读完这一条就不必再点开原文。

- **360-600 汉字**，4-6 段，段间空一行
- 结构：一句强判断开头 → 按时序把事情讲完整（主体，占一半以上篇幅）→
  讲清机制或背景 → SO WHAT 二阶推演
- 必须交代的关键要素（融进叙事，不列清单）：谁、发生了什么、什么时候、
  多大规模、怎么发生的、现在什么状态、还有什么没答案
- **不带链接、不带话题标签、不提媒体来源**（事件当事人姓名照常写）
- 不提问、不求转发、不喊单、不预测价格
- 生成失败或产出不合格（太短、超长、清理后为空）时**自动回落**到模板拼装
  （标题 + 一句摘要），并在 run report 记一条 `x_compose_fallback`

写长推需要更多素材，光靠摘要写不出时间线，所以除了富化后的各字段，
...
URL 一律按 23 计。`max_post_chars` 是加权上限，默认 1600（600 汉字约合 1200，
留了余量），标准账号请改成 280 —— 但标准账号发不了长推，那种情况应该同时把
`compose` 改成 `template`。

产出低于 **500 加权单位**（约 250 汉字）会被判为生成失败并回落到模板。
这个下限是刻意的：按叙事标准，写不到这个长度说明事情根本没讲完。
...
- `max_post_chars`：加权字数上限，默认 1600；标准账号设 280 并把 `compose` 改成 `template`
